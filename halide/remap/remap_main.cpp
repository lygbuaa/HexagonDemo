...
Buffer<uint8_t> test_halide_remap(Buffer<uint8_t> in, Buffer<uint16_t> tableX, Buffer<uint16_t> tableY) {
    const uint16_t inW = in.width();
    const uint16_t inH = in.height();
    const uint16_t inC = in.channels();
    Buffer<uint8_t> out(inW, inH, inC);
...
//    halide_remap(in, tableX, tableY, out);
    halide_remap(in, out);
...
int main(int argc, char **argv) {
    ...
    Buffer<uint8_t> remapHalide = test_halide_remap(input, tableX, tableY);
